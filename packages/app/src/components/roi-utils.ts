...
export function steamPriceMap(result: PriceScanResult | undefined) {
  return new Map((result?.items ?? []).flatMap((item) => {
    const quote = item.quotes.find((candidate) => candidate.source === "steam") ?? item.quotes[0];
    const amount = quote?.adjustedAmountMinor ?? quote?.amountMinor;
    return amount === undefined ? [] : [[item.marketName, amount] as const];
  }));
}

export async function scanPriceMap(marketNames: readonly string[], scan: (marketNames: string[], appId?: number) => Promise<PriceScanResult | undefined>) {
  ...
  for (const result of results) {
    for (const [name, amount] of steamPriceMap(result)) prices.set(name, amount);
  }
  return prices;
}

export function expectedReturn(outcomes: ReadonlyArray<{ marketName?: string; probability: number }>, prices: ReadonlyMap<string, number>, costMinor?: number): ReturnEstimate {
  let expectedValueMinor = 0;
  let pricedOutcomes = 0;
  for (const outcome of outcomes) {
    const price = outcome.marketName ? prices.get(outcome.marketName) : undefined;
  ...
  return {
    expectedValueMinor,
    costMinor,
    roiPercent: costMinor && costMinor > 0 ? (expectedValueMinor - costMinor) / costMinor * 100 : undefined,
    pricedOutcomes,
