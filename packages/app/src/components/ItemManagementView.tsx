  ...
  return (
    <div class="space-y-5">
      ...
      <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <OperationFormCard title="Delete item" actionLabel="Delete item" actionClass="border-rose-500/40 bg-rose-600/80" pending={pending()} onExecute={() => run(() => props.onDeleteItem(deleteInput()))}>
        ...
        <OperationFormCard title="Use item" actionLabel="Use item" actionClass="border-cyan-500/40 bg-cyan-600/80" pending={pending()} onExecute={() => run(() => props.onUseItem(useItemInput()))}>
        ...
        <OperationFormCard title="Use multiple items" actionLabel="Use multiple" actionClass="border-cyan-500/40 bg-cyan-600/80" pending={pending()} onExecute={() => run(() => props.onUseMultipleItems({ itemIds: parseMultipleIDs() }))}>
        ...
        <OperationFormCard title="Gift item" actionLabel="Send gift" actionClass="border-cyan-500/40 bg-cyan-600/80" pending={pending()} onExecute={() => run(() => props.onGiftItem(giftInput()))}>
...
function OperationFormCard(props: { title: string; actionLabel: string; actionClass: string; pending: boolean; onExecute: () => void; children: JSX.Element | string | number | null | undefined }) {
  return (
    <section class="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm">
      ...
      <div class="mt-4 space-y-3 text-sm">
        {props.children}
        <button class={`rounded-md border px-3 py-2 text-white disabled:opacity-60 ${props.actionClass}`} disabled={props.pending} onClick={() => props.onExecute()}>
        {props.actionLabel}
        </button>
      </div>
    </section>
