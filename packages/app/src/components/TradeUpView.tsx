...
import {
  ...
  effectiveFloat,
  resolveTradeUpLocally,
  tradeUpInputCount,
  type TradeUpResolver,
} from "./trade-up-utils.js";
...
import { SelectedTradeUpItemCard as SelectedItemCard } from "./trade-up-selected-item-card.js";
...
function EligibleItemButton(props: {
  ...
  return (
    <button
      ...
      class={`text-left rounded-2xl border-2 p-4 transition ${rarityBorderClass(props.item.rarity)} ${props.selected ? "bg-cyan-500/10 ring-1 ring-cyan-400/40" : "bg-slate-900/80 hover:bg-slate-800/90"} disabled:cursor-not-allowed disabled:opacity-35`}
      onClick={props.onSelect}
    >
      <div class="flex items-start gap-3">
        <Show when={props.item.imageUrl}>
          <img
            class="h-14 w-20 shrink-0 rounded bg-slate-950 object-contain"
        ...
        <div class="min-w-0">
          <strong class="block truncate text-slate-100">
      ...
      <div class="mt-3 flex justify-between text-xs">
        <span class="font-mono text-slate-300">
          Float {formatFloat(props.item.paintWear!)}
        </span>
        <span class="font-mono text-cyan-300">
          Effective {formatFloat(effectiveFloat(props.item))}
        </span>
      </div>
    </button>
  );
...
function OutcomeCard(props: {
  outcome: ReturnType<typeof calculateTradeUpOutcomes>[number];
}) {
  return (
    <article
      ...
      <div class="flex gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="mt-1 text-xl font-semibold text-cyan-300">
            {(props.outcome.probability * 100).toFixed(2)}%
          </p>
        </div>
      </div>
      <div class="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div>
          ...
          <p class="font-mono text-slate-200">
            {formatFloat(props.outcome.predictedWear)}
          </p>
        </div>
        <div>
          <p class="text-slate-500">Output caps</p>
          <p class="font-mono text-slate-200">
            {formatFloat(props.outcome.wearMin ?? 0)}–
            {formatFloat(props.outcome.wearMax ?? 1)}
          </p>
        </div>
      </div>
    </article>
...
export function TradeUpView(props: TradeUpViewProps) {
  ...
  const requiredCount = createMemo(() =>
    firstItem() ? tradeUpInputCount(firstItem()!) : 10,
  );
  ...
  const averageFloat = createMemo(() =>
    selectedItems().length
      ? selectedItems().reduce((sum, item) => sum + (item.paintWear ?? 0), 0) /
        selectedItems().length
      : 0,
  );
  const averageEffective = createMemo(() =>
    selectedItems().length
      ? selectedItems().reduce((sum, item) => sum + effectiveFloat(item), 0) /
        selectedItems().length
      : 0,
  );
  ...
  const playPreview = async () => {
    ...
    const resolver: TradeUpResolver =
      props.resolveTradeUp ??
    ...
    const toRevealItem = (outcome: (typeof available)[number]): RevealItem => ({
      name: displayName(outcome),
      imageUrl: outcome.imageUrl,
      rarity: outcome.rarity,
      kind: outcome.kind,
      wear: outcome.predictedWear,
      wearMin: outcome.wearMin,
      wearMax: outcome.wearMax,
    });
    const candidates = available.map(toRevealItem);
    if (mode !== "none" && mode !== "contract-none")
      setPreview({ result: candidates[0]!, ready: false, candidates });
    await fromAppPromise(resolver(request), "Trade-up resolution failed").match(
      (picked) => {
        setPreviewStatus("");
        if (mode === "none") return;
  ...
  const canSelect = (item: InventoryItemDto) => {
    ...
    return (
      ...
      !first ||
      (compatibleTradeUpItem(first, item) &&
    ...
    <div class="space-y-5">
      <Show
        ...
        fallback={
          <RevealAnimation
            ...
            title="Trade-up preview"
            candidates={preview()?.candidates ?? []}
            result={preview()?.result ?? { name: "Trade-up result" }}
            onComplete={() => setPreview(undefined)}
          />
        }
      >
      ...
      <header>
        ...
        <p class="mt-2 max-w-3xl text-sm text-slate-400">
          Build a contract from your inventory and preview wear and odds
          locally. This planner does not contact the Game Coordinator or execute
          a trade-up.
        </p>
      </header>
      <section class="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm">
        <div class="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-xl font-semibold text-slate-100">
              {selectedItems().length} / {requiredCount()} items
            </p>
            <Show when={firstItem()}>
              <p class="mt-1 text-xs text-slate-400">
                {firstItem()!.rarity} ·{" "}
                {firstItem()!.isStatTrak ? "StatTrak™" : "Regular"}
                {requiredCount() === 5 ? " · knife/glove contract" : ""}
              </p>
            </Show>
          </div>
          <Show when={selectedItems().length > 0}>
            <button
              type="button"
              class="rounded-md border border-slate-700 px-3 py-2 text-sm text-slate-300 hover:border-cyan-400/40"
        ...
        <div class="mt-4 grid gap-3 sm:grid-cols-2">
          <div class="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
            ...
            <p class="mt-1 font-mono text-lg text-slate-100">
              {selectedItems().length ? formatFloat(averageFloat()) : "—"}
            </p>
          </div>
          <div class="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
            <p class="text-xs uppercase tracking-wide text-slate-500">
              Average effective float
            </p>
            <p class="mt-1 font-mono text-lg text-cyan-300">
              {selectedItems().length ? formatFloat(averageEffective()) : "—"}
            </p>
            <p class="mt-1 text-xs text-slate-500">
              Each float normalized within its finish’s wear caps.
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <p class="mt-1 text-sm text-slate-400">
          Your first pick locks rarity and StatTrak™ type for the contract.
        </p>
        <Show
          when={eligibleItems().length > 0}
          fallback={
            <div class="mt-3 rounded-xl border border-dashed border-slate-700 p-6 text-sm text-slate-400">
              No trade-up eligible skins with float and outcome metadata were
              found.
            </div>
          }
        >
      ...
      <Show when={selectedItems().length === requiredCount()}>
        <section class="rounded-2xl border border-cyan-500/25 bg-cyan-950/10 p-4">
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p class="mt-1 text-sm text-slate-400">
                Each collection receives odds in proportion to its input count;
                outcomes within that collection split its share evenly.
              </p>
              <Show when={previewStatus()}>
                <p class="mt-2 text-xs text-amber-300">{previewStatus()}</p>
              </Show>
            </div>
            <button
              type="button"
