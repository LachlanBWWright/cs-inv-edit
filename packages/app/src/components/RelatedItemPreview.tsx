...
export function RelatedItemPreview(props: { item: RelatedItemDto; context?: RelatedItemPreviewContext; probability?: number; onRequestMarketPreview?: (marketName: string) => Promise<RelatedItemDto | undefined> }) {
  ...
  const requestMarketPreview = async () => {
    ...
    if (!marketName || item().price || marketRequested() || !props.onRequestMarketPreview) return;
  ...
  return <details class={`group rounded-xl border-2 bg-slate-900/80 ${rarityBorderClass(item().rarity)}`} onToggle={(event) => { if (event.currentTarget.open) void requestMarketPreview(); }}>
    ...
    <div class="border-t border-slate-800/80 px-3 pb-3 pt-3 text-xs text-slate-400">
      ...
      <Show when={item().listingName} fallback={<Show when={item().marketName}><a class="mt-3 inline-block font-medium text-sky-300 underline decoration-sky-500/50 underline-offset-4 hover:text-sky-200" href={steamMarketSearchURL(item().marketName!)} target="_blank" rel="noreferrer">Search Steam Market ↗</a></Show>}>
        <a class="mt-3 inline-block font-medium text-sky-300 underline decoration-sky-500/50 underline-offset-4 hover:text-sky-200" href={steamMarketURL(item().listingName!)} target="_blank" rel="noreferrer">Open exact Steam listing ↗</a>
      </Show>
