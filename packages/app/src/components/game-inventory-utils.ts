...
export function economyCategoryOptions(game: EconomyGame, snapshot?: GameInventorySnapshot) {
...
export function virtualInventoryWindowChanged(itemCount: number, width: number, height: number, previousScrollTop: number, nextScrollTop: number, compactMode: "icons" | "concise" | "detailed") {
  const previous = calculateVirtualInventoryWindow(itemCount, width, height, previousScrollTop, compactMode);
  const next = calculateVirtualInventoryWindow(itemCount, width, height, nextScrollTop, compactMode);
  return previous.firstItem !== next.firstItem || previous.lastItem !== next.lastItem;
}
