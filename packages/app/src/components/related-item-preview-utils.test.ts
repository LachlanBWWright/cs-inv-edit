...
import { cappedWearDistribution, containerItemOdds, generateCappedWear, weightedRandomItem } from "./related-item-preview-utils.js";
...
describe("weightedRandomItem", () => {
  it("selects adjacent rarity tiers at a 5:1 ratio and divides a tier between its items", () => {
    const items = [{ name: "A", rarity: "Restricted" }, { name: "B", rarity: "Restricted" }, { name: "C", rarity: "Classified" }];
...
describe("generateCappedWear", () => {
  ...
  it("can select the final wear bracket", () => {
    ...
    expect(generateCappedWear(0, 1, () => rolls.shift() ?? 0)).toBeCloseTo(0.725);
  });
});
