  ...
  return (
    <div class="rounded-2xl border border-slate-800 bg-slate-950/50 p-3 text-sm text-slate-300">
      <div class="flex items-center justify-between gap-3">
        <span class="font-mono text-xs text-slate-400">{props.receipt.operationId}</span>
        <span class="rounded-full border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-slate-200">{formatState(props.receipt.state)}</span>
      </div>
      <p class="mt-2">{props.receipt.type}</p>
      <p class="mt-1 text-xs text-slate-400">{formatTimestamp(props.receipt.createdAt)}</p>
    </div>
  );
}
  ...
  return (
    <div class="rounded-2xl border border-slate-800 bg-slate-950/50 p-3 text-sm text-slate-300">
      <div class="flex items-center justify-between gap-3">
        ...
        <span class="rounded-full border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-slate-200">{formatState(props.event.state)}</span>
      </div>
      <p class="mt-2">{props.event.message}</p>
      <p class="mt-1 text-xs text-slate-400">{formatTimestamp(props.event.createdAt)}</p>
    </div>
  );
}
  ...
  return (
    <div class="space-y-5">
      <header>
        ...
        <p class="mt-2 max-w-2xl text-sm text-slate-400">A compact operation log that tracks receipts, state progression, and emitted events.</p>
      </header>
        ...
        <Card>
          <CardContent>
            ...
            <div class="mt-4 space-y-2">
              <For each={props.receipts ?? []}>{(receipt) => <ReceiptCard receipt={receipt} />}</For>
            </div>
          </CardContent>
        </Card>
        <Card>
