...
export function supportsPullToRefresh() {
  return typeof globalThis.matchMedia === "function" && globalThis.matchMedia("(hover: none) and (pointer: coarse)").matches;
}
...
export function PullToRefresh(props: PullToRefreshProps) {
  ...
  onMount(() => {
    ...
    const touchMove = (event: TouchEvent) => {
      ...
      const distance = Math.max(0, (event.touches[0]?.clientY ?? startY) - startY);
  ...
  return (
    <div
      ref={(node) => { element = node; props.ref?.(node); }}
      class={`relative ${props.class ?? ""}`}
      onScroll={props.onScroll}
    >
      <Show when={supportsPullToRefresh()}>
        <div class="pointer-events-none absolute inset-x-0 top-0 z-10 flex justify-center transition-transform duration-150" style={{ transform: `translateY(${pullDistance() - 44}px)` }} aria-hidden="true">
          <div class="flex h-9 items-center gap-2 rounded-full border border-cyan-400/20 bg-slate-950/95 px-3 text-xs font-medium text-cyan-200 shadow-lg">
            <span class={`h-4 w-4 rounded-full border-2 border-slate-700 border-t-cyan-300 ${refreshing() ? "animate-spin" : ""}`} />
