...
export function Button(props: ButtonProps) {
  ...
  const base = "inline-flex items-center justify-center rounded-lg border font-medium transition focus:outline-none focus:ring-2 focus:ring-cyan-400/60 disabled:cursor-not-allowed disabled:opacity-60";
  const styles = {
  ...
  return (
    <button type={props.type ?? "button"} class={cn(base, styles[variant], sizes[size], props.class)} onClick={props.onClick} disabled={props.disabled}>
      {props.children}
    </button>
  );
}
