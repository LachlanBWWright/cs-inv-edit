  ...
  return (
    <div class="mt-2 flex flex-wrap items-center gap-1.5">
      ...
      <Show when={props.item.customName}>
        <span class="rounded bg-cyan-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-cyan-200" title={`Name Tag: ${props.item.customName}`}>Name Tag</span>
      </Show>
      ...
      <For each={props.item.appliedItems ?? []}>{(applied) => (
        <span class="inline-flex h-4 w-4 items-center justify-center overflow-hidden rounded-sm border border-slate-600 bg-slate-800 text-[8px] font-bold uppercase text-slate-300" title={`${applied.kind}: ${applied.name}${applied.slot === undefined ? "" : ` (slot ${applied.slot + 1})`}${applied.kind === "sticker" && applied.wear !== undefined ? ` · ${Math.round(Math.max(0, Math.min(1, applied.wear)) * 100)}% scraped` : ""}`}>
          <Show when={applied.imageUrl} fallback={<span aria-label={`${applied.kind}: ${applied.name}`}>{applied.kind === "charm" ? "C" : applied.kind === "patch" ? "P" : "S"}</span>}>
