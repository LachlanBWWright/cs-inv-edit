  ...
  return (
    <header class="sticky top-0 z-20 flex flex-wrap items-center gap-2 border-b border-slate-800/80 bg-slate-950/90 px-3 py-2 backdrop-blur lg:px-4">
        ...
        >
          ...
          >
            <span class="min-w-0 flex-1 leading-tight">
              <span class="block text-[9px] font-semibold uppercase tracking-[0.16em] text-slate-500">
                {currentGroup()}
              </span>
              <span class="block truncate text-sm font-semibold text-slate-100">
                {modeDetails[currentMode()].label}
              </span>
            </span>
            <svg
              class={`h-4 w-4 shrink-0 text-slate-400 transition-transform ${modeMenuOpen() ? "rotate-180" : ""}`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
          ...
          <Show when={modeMenuOpen()}>
            ...
            >
              <For each={modeGroups}>
                {(group, groupIndex) => {
                  const visibleModes = () =>
                    group.modes.filter((mode) => enabledModes().has(mode));
                  return (
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        >
          <button
            class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 bg-slate-900/80 text-slate-200 transition hover:border-cyan-400/50 hover:text-cyan-100"
