...
function StorageUnitCard(props: { unit: InventoryItemDto; selectedItemId: string | undefined; onSelect: () => void; onRunOperation: (type: string, input?: unknown) => Promise<void> }) {
  const isSelected = () => props.selectedItemId === props.unit.id;

  return (
    <article class={`rounded-2xl border p-4 shadow-sm ${isSelected() ? "border-cyan-500/50 bg-cyan-500/10" : "border-slate-800 bg-slate-900/80"}`}>
      <div class="flex items-start justify-between gap-3">
        <div>
          <h3 class="font-semibold text-slate-100">{props.unit.name}</h3>
          <p class="mt-1 text-sm text-slate-400">Count: {props.unit.storageCount ?? 0}</p>
        </div>
        <button class="rounded-md border border-slate-700 bg-slate-950/70 px-3 py-1 text-sm text-slate-200 transition hover:border-cyan-400/40" onClick={() => props.onSelect()}>Select</button>
      </div>
      <div class="mt-4 flex flex-wrap gap-2">
        <button class="rounded-md border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-400/40" onClick={() => props.onRunOperation("storage.load", { casketId: props.unit.id })}>Load contents</button>
        <button class="rounded-md border border-cyan-500/40 bg-cyan-600/80 px-3 py-2 text-sm text-white transition hover:bg-cyan-500" onClick={() => props.onRunOperation("storage.move-in", { casketId: props.unit.id, itemId: props.selectedItemId ?? "" })}>Move item in</button>
        <button class="rounded-md border border-rose-500/40 bg-rose-600/80 px-3 py-2 text-sm text-white transition hover:bg-rose-500" onClick={() => props.onRunOperation("storage.move-out", { casketId: props.unit.id, itemId: props.selectedItemId ?? "" })}>Move item out</button>
      </div>
    </article>
  );
}
  ...
  return (
    <div class="space-y-5">
      <header class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button class="rounded-md border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-400/40 hover:bg-slate-800" onClick={() => props.onRefresh()}>Reload snapshot</button>
      </header>
      <Show when={status()}>
      ...
      <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <div class="space-y-3">
          <For each={storageUnits()}>{(unit) => <StorageUnitCard unit={unit} selectedItemId={selectedItem()?.id} onSelect={() => setSelectedItem(unit)} onRunOperation={runOperation} />}</For>
        </div>
