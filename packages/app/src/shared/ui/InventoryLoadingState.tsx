...
function InventorySkeletonGrid() {
  return (
    <div
      class="grid w-full items-start gap-3"
      style={inventorySkeletonGridStyle}
      aria-hidden="true"
    >
      <For each={skeletonCards}>
        {() => (
          <div class="flex h-[300px] flex-col overflow-hidden rounded-2xl border-2 border-slate-800 bg-slate-950">
            <div class="h-36 shrink-0 bg-slate-900" />
            <div class="flex flex-1 flex-col px-3 py-3">
              <div class="h-4 w-4/5 rounded bg-slate-700" />
              <div class="mt-2 h-3 w-3/5 rounded bg-slate-800" />
              <div class="mt-3 flex gap-1.5">
                <div class="h-4 w-14 rounded bg-slate-800" />
                <div class="h-4 w-20 rounded bg-slate-800" />
              </div>
              <div class="mt-auto h-3 w-full rounded bg-slate-800" />
            </div>
          </div>
        )}
      </For>
...
function CatalogSkeletonGrid() {
  return (
    <div
      class="grid flex-1 items-start gap-4 md:grid-cols-2 2xl:grid-cols-3"
      aria-hidden="true"
    >
      <For each={skeletonCards}>
        {() => (
...
export function InventoryLoadingState(props: InventoryLoadingStateProps) {
  ...
  return (
    <section
      class={`w-full animate-pulse ${catalog() ? "flex min-h-[calc(100vh-12rem)] flex-col gap-4" : ""}`}
      role="status"
      aria-label={props.title}
      aria-busy={props.active}
    >
      <span class="sr-only">{props.title}</span>
      <Show when={catalog()} fallback={<InventorySkeletonGrid />}>
