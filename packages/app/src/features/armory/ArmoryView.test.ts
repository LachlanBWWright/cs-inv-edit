...
import { generateRevealMiss, STATTRAK_ODDS } from "../../shared/ui/RevealAnimation.js";
...
describe("Armory purchase timeout", () => {
  it("rejects a purchase that remains pending for 40 seconds", async () => {
    vi.useFakeTimers();
    const result = withArmoryPurchaseTimeout(
...
describe("Armory reveal items", () => {
  ...
  it("allows StatTrak misses only for weapon case candidates at exactly one in ten", () => {
    const [caseCandidate] = armoryRevealCandidates(
      [{ name: "Case skin", kind: "weapon_skin" }],
      "stattrak",
    );
    const [collectionCandidate] = armoryRevealCandidates(
      [{ name: "Collection skin", kind: "weapon_skin" }],
      "regular",
    );

    expect(
      generateRevealMiss(caseCandidate!, () => STATTRAK_ODDS - Number.EPSILON)
        .isStatTrak,
    ).toBe(true);
    expect(
      generateRevealMiss(caseCandidate!, () => STATTRAK_ODDS).isStatTrak,
    ).toBe(false);
    expect(generateRevealMiss(collectionCandidate!, () => 0).isStatTrak).toBe(
      false,
    );
  });

  it("makes every souvenir-package skin Souvenir and never StatTrak", () => {
    const [candidate] = armoryRevealCandidates(
      [{ name: "Package skin", kind: "weapon_skin" }],
      "souvenir",
    );
