...
function NavbarModeButton(props: {
  mode: AppMode;
  selected: boolean;
  ...
  return (
    <button
      class={`flex w-full items-center gap-3 rounded-sm border px-2.5 py-2 text-left transition ${props.selected ? "border-cyan-400/25 bg-cyan-950" : "border-transparent hover:border-slate-700/70 hover:bg-slate-800"}`}
      role="menuitemradio"
...
export function NavbarModeGroup(props: {
  group: ModeGroup;
  index: number;
