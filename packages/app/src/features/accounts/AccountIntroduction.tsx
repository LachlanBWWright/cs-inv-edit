...
function FeatureGroupCard(props: { group: (typeof modeGroups)[number] }) {
  return (
    <section aria-label={props.group.label}>
      <h5 class="text-xs font-medium uppercase tracking-wider text-slate-500">
        {props.group.label}
      </h5>
      <ul class="mt-2 space-y-1.5 text-sm text-slate-300">
        <For each={props.group.modes}>
          {(mode) => (
            <li>
              <span class="font-medium text-slate-200">{modeDetails[mode].label}</span>
              <span class="text-slate-500">{` — ${modeDetails[mode].description}`}</span>
            </li>
          )}
        </For>
      </ul>
    ...
    <section class="space-y-8 py-4" aria-labelledby="application-introduction">
      ...
      <div class="space-y-4">
        ...
        <div class="grid gap-x-8 gap-y-5 sm:grid-cols-2">
          <For each={modeGroups}>
            {(group) => <FeatureGroupCard group={group} />}
          </For>
        </div>
      </div>
    </section>
