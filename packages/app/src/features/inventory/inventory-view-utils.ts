...
export function rarityRank(rarity: string | undefined) {
  ...
  if (
    ...
    return 7;
  if (["ancient", "covert", "extraordinary", "master"].includes(value))
    return 6;
  if (["legendary", "classified", "exotic", "superior"].includes(value))
    return 5;
  if (["mythical", "restricted", "remarkable", "exceptional"].includes(value))
    return 4;
  if (
    [
      "rare",
    ...
    return 3;
  if (["uncommon", "industrial grade", "medium grade"].includes(value))
    return 2;
...
export function itemDisplayName(item: InventoryItemDto) {
  return (
    ...
    item.marketName ||
    item.name ||
    `CS2 item #${item.defindex}`
  );
}

export function isOpenableContainer(item: InventoryItemDto | undefined) {
  if (!item) return false;
  if (item.kind === "container" || (item.containerItems?.length ?? 0) > 0)
    return true;
  return /(?:container|capsule|case|graffiti box)/i.test(
...
function splitInventoryName(item: InventoryItemDto) {
  ...
  return {
    name:
      (qualifierMatch
        ? rawName.slice(0, qualifierMatch.index).trim()
        : rawName) || fullName,
...
export function compactItemMeta(item: InventoryItemDto) {
  ...
  const type =
    item.kind === "weapon_skin"
      ? itemWeaponName(item)
      : parts.type || itemKindLabel(item.kind);
...
export function sortInventoryItems(
  ...
  return [...items].sort((left, right) => {
    if (sort === "price-low" || sort === "price-high") {
      const price = (item: InventoryItemDto) =>
        item.marketable === false
          ? 0
          : (marketPrices.get(item.marketName ?? "") ?? 0);
      const comparison = price(left) - price(right);
      return (
        (sort === "price-low" ? comparison : -comparison) ||
        itemDisplayName(left).localeCompare(itemDisplayName(right))
      );
    }
    if (sort === "float-low" || sort === "float-high") {
      if (left.paintWear === undefined)
        return right.paintWear === undefined ? 0 : 1;
    ...
    if (sort === "rarity-low" || sort === "rarity-high") {
      ...
      return (
        (sort === "rarity-low" ? comparison : -comparison) ||
        itemDisplayName(left).localeCompare(itemDisplayName(right))
      );
    }
...
export function rarityDisplayLabel(rarity: string | undefined) {
  if (!rarity) return "";
  const normalized = rarity.trim().toLowerCase().replace(/\s+/g, " ");
  switch (normalized) {
    case "common":
    case "consumer grade":
    case "base grade":
