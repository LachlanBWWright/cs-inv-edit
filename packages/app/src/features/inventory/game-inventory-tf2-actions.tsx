    ...
    <>
                <Show when={item().game === "tf2"}>
                  <div class="mt-4 space-y-2 border-t border-slate-800 pt-4">
                    ...
                    <Show when={props.settings?.featureFlags.enableTf2Customization && selectedTF2Item() && props.onOperation}>
                      <TF2DecalTool
                        target={selectedTF2Item()!}
                        items={(snapshot()?.items ?? []).filter(
                          (candidate): candidate is Extract<EconomyInventoryItemDto, { game: "tf2" }> => candidate.game === "tf2",
                        )}
