    ...
    >
      ...
      >
        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          <For each={props.items}>{(item) => <ItemTile item={item} />}</For>
        </div>
      </Show>
    </div>
  );
    ...
    <Card class="overflow-hidden p-0">
      <div class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-800/80 px-4 py-3">
        <div class="flex min-w-0 items-center gap-3">
          ...
          >
            <Show
              when={props.trade.partnerAvatarUrl}
              fallback={(props.trade.partnerName || "?")
                .slice(0, 1)
                .toUpperCase()}
            >
              <img
      ...
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <Show when={tab() === "history"}>
            <label class="text-sm text-slate-400">
              ...
              <select
                ...
                value={accountScope()}
                onChange={(event) => setAccountScope(event.currentTarget.value)}
              >
                <option value="all">All available accounts</option>
                <For each={props.accounts?.accounts ?? []}>
                  {(account) => (
                    <option value={account.steamId}>
                      {account.accountName}
                    </option>
                  )}
                </For>
          ...
          <Button disabled={busy()} onClick={() => void refresh()}>
            {busy() ? "Refreshing…" : "Refresh trades"}
          </Button>
        </div>
      </div>
          ...
          fallback={
            <div class="rounded-2xl border border-dashed border-slate-700/80 px-6 py-14 text-center">
              <p class="text-lg font-medium text-slate-300">
                No {tab() === "history" ? "trade history" : `${tab()} offers`}
              </p>
