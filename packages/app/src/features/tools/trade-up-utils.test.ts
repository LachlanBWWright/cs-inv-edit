...
const item = (
  ...
  paintWearMin: 0,
  paintWearMax: 0.1,
  tradeUpItems: candidates.map((name) => ({
    name,
    wearMin: 0.1,
    wearMax: 0.5,
  })),
...
describe("trade-up calculations", () => {
  ...
  it("weights each input equally and candidates within it equally", () => {
    ...
    expect(
      outcomes.find((outcome) => outcome.name === "Shared")?.probability,
    ).toBeCloseTo(0.75);
    expect(
      outcomes.find((outcome) => outcome.name === "A only")?.probability,
    ).toBeCloseTo(0.25);
  ...
  it("keeps local resolution behind the same request boundary as a future GC resolver", () => {
    const outcomes = calculateTradeUpOutcomes([
      item("a", "A", ["First", "Second"]),
    ]);
    expect(
      resolveTradeUpLocally({ itemIds: ["a"], outcomes }, () => 0)?.name,
    ).toBe("First");
    expect(
      resolveTradeUpLocally({ itemIds: ["a"], outcomes }, () => 0.99)?.name,
    ).toBe("Second");
  });
});
