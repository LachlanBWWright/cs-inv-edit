...
export const inventoryItemSchema = z.object({
  ...
  kind: z.enum(["weapon_skin", "sticker_item", "container", "storage_unit", "tool_item", "cs2_econ_item", "unknown"]), defindex: z.number().optional(), paintWear: z.number().optional(), paintWearMin: z.number().optional(), paintWearMax: z.number().optional(), storageCount: z.number().optional(), casketId: z.string().optional(), collection: z.string().optional(),
  collectionItems: z.array(relatedItemSchema).optional(), tradeUpItems: z.array(relatedItemSchema).optional(), containerItems: z.array(relatedItemSchema).optional(), exterior: z.string().optional(), rarity: z.string().optional(), storageLocation: z.string().optional(), toolType: z.string().optional(),
...
export const economyGameSchema = z.enum(["tf2", "dota2"]);
const economyItemBaseShape = {
  contextId: z.string().optional(), assetId: z.string(), classId: z.string().optional(), instanceId: z.string().optional(), definitionId: z.number().int().nonnegative().optional(),
  name: z.string(), marketName: z.string().optional(), imageUrl: z.string().url().optional(), quantity: z.number().int().positive(), type: z.string().optional(), rarity: z.string().optional(), quality: z.string().optional(), tradable: z.boolean(), marketable: z.boolean(),
  tags: z.array(z.object({ category: z.string(), internalName: z.string(), name: z.string() })), descriptions: z.array(z.string()).optional(),
};
const economyDetailsBaseShape = {
  level: z.number().int().nonnegative(), qualityId: z.number().int().nonnegative(), inventoryPosition: z.number().int().nonnegative(), originId: z.number().int().nonnegative(), style: z.number().int().nonnegative(), flags: z.number().int().nonnegative(),
  customName: z.string().optional(), customDescription: z.string().optional(), attributes: z.record(z.string(), z.number().int().nonnegative()),
  attributeBytes: z.record(z.string(), z.string().regex(/^[0-9a-f]*$/)).optional(), equippedStates: z.array(z.object({ class: z.number().int().nonnegative(), slot: z.number().int().nonnegative() })).optional(), interiorItemId: z.string().regex(/^\d+$/).optional(),
};
const tf2EconomyItemSchema = z.object({ ...economyItemBaseShape, game: z.literal("tf2"), appId: z.literal(440), details: z.object({ ...economyDetailsBaseShape, game: z.literal("tf2"), schemaQuality: z.string().optional(), equipSlot: z.string().optional(), usableClasses: z.array(z.string()).optional(), capabilities: z.record(z.string(), z.string()).optional() }) });
const dota2EconomyItemSchema = z.object({ ...economyItemBaseShape, game: z.literal("dota2"), appId: z.literal(570), details: z.object({ ...economyDetailsBaseShape, game: z.literal("dota2"), hero: z.string().optional(), slot: z.string().optional() }) });
const snapshotBaseShape = { refreshedAt: z.string(), status: z.enum(["ready", "requires_connection", "loading", "error"]), message: z.string().optional(), error: z.string().optional(), schemaRevision: z.string().optional(), diagnostics: z.array(z.string()) };
export const gameInventorySnapshotSchema: z.ZodType<GameInventorySnapshot> = z.discriminatedUnion("game", [
  z.object({ ...snapshotBaseShape, game: z.literal("tf2"), appId: z.literal(440), items: z.array(tf2EconomyItemSchema) }),
  z.object({ ...snapshotBaseShape, game: z.literal("dota2"), appId: z.literal(570), items: z.array(dota2EconomyItemSchema) }),
]);
export const settingsDataSchema: z.ZodType<SettingsData> = z.object({
  backendUrl: z.string(), validationMode: z.boolean(), sacrificialAccountMode: z.boolean(), armoryPurchasePacingSeconds: z.number().int().min(1).max(60),
  featureFlags: z.object({ enableStorageMutations: z.boolean(), enableContainerOpening: z.boolean(), enableInventoryDebug: z.boolean(), enableTradeups: z.boolean(), enableStickerExtract: z.boolean(), enableNameTags: z.boolean(), enableItemDeletion: z.boolean(), enableStatTrakSwap: z.boolean(), enableStrangeParts: z.boolean(), enableItemUse: z.boolean(), enableToolApplication: z.boolean(), enableGifting: z.boolean(), enableArmoryRead: z.boolean().optional(), enableArmoryRedemption: z.boolean().optional(), enableTf2Inventory: z.boolean().default(false), enableDota2Inventory: z.boolean().default(false) }),
