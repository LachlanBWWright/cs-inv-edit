...
func (p *Provider) fetchPage(ctx context.Context, steamID string, game Game, startAssetID, webAccessToken string) (page, error) {
	...
	if webAccessToken != "" {
		cookieValue := url.QueryEscape(steamID + "||" + webAccessToken)
		req.AddCookie(&http.Cookie{Name: "steamLoginSecure", Value: cookieValue, Path: "/", Secure: true, HttpOnly: true})
	}
	resp, err := p.do(req)
	if err != nil {
