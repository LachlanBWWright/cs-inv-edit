...
function App() {
  ...
  const stickerPlanSummary = createMemo<Summary<{ confidence: string; notes: string[] }> | undefined>(() => {
  ...
  const tradeUpPlanSummary = createMemo<Summary<{ collection: string; averageWear: string; predictedTier: string; outputTheme: string }> | undefined>(() => {
  ...
  const storagePlanSummary = createMemo<Summary<{ summary: string; targetFreeSlots: number }> | undefined>(() => {
