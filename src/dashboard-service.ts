...
export const loadDashboardData = async (): Promise<
  ResultTuple<DashboardData, DashboardError>
> => {
  await wait(180);
  ...
  const data: DashboardData = {
    ...
    platformFocus:
      "CS2 inventory edit · live review shell for sticker, trade-up, and storage workflows",
    inventory,
    storageUnits: [
      {
        id: "sto-001",
        name: "Operations Queue",
        zone: "Trade-up staging",
        capacity: 80,
        occupied: 62,
        theme: "Fast-moving route for aspirational baskets.",
      },
      {
        id: "sto-002",
        name: "Premium Showcase",
        zone: "High-value preservation",
        capacity: 50,
        occupied: 48,
        theme: "Tight capacity calls for deliberate routing.",
      },
    ],
    activity: [
      {
        id: "activity-1",
        title: "Sticker presets loaded",
        detail: "Three review modes are ready for manual checking.",
        status: "Completed",
        timestamp: "2m ago",
      },
      {
        id: "activity-2",
        title: "Trade-up basket staged",
        detail: "10-item overview is prepared for a single collection review.",
        status: "Queued",
        timestamp: "8m ago",
      },
      {
        id: "activity-3",
        title: "Storage watch enabled",
        detail:
          "Premium items are flagged for capacity pressure before any move.",
        status: "Review",
        timestamp: "12m ago",
      },
    ],
    statusFlags: [
      {
        ...
        value: "Preview shell is connected to a mock snapshot.",
        tone: "Stable",
      },
      {
        label: "Storage pressure",
        value: "Premium showcase is 96% occupied.",
        tone: "Watch",
      },
      {
        label: "Policy gate",
        value: "Write actions remain human-approved.",
        tone: "Risk",
      },
    ],
    complianceNotes: [
      {
        title: "Manual approval preserved",
        detail:
          "Every action remains reviewable before any write path is touched.",
      },
      {
        title: "Collection consistency",
        detail:
          "Trade-up baskets stay isolated to a single collection and rarity for clear review.",
      },
      {
        title: "Storage queue discipline",
        detail: "Capacity pressure is surfaced before any move is reserved.",
      },
    ],
...
export const buildStickerPlan = (
  item: InventoryItem,
  preset: string,
): Result<StickerPlan, DashboardError> => {
  if (item.inStorage) {
    return err(
      createInvalidSelectionError(
        "Move the item out of storage before applying a sticker refinement plan.",
      ),
    );
  }
...
export const buildTradeUpPlan = (
  items: InventoryItem[],
): Result<TradeUpPlan, DashboardError> => {
  if (items.length !== 10) {
    return err(
      createInvalidSelectionError(
        "Select exactly 10 items to review a trade-up basket.",
      ),
    );
  }
  ...
  const hasMixedCollection = items.some(
  ...
  if (
    ...
    return err(
      createInvalidSelectionError(
        "Trade-up baskets must stay on one collection and one rarity for a clean review surface.",
      ),
    );
  }

  const averageWear =
    items.reduce((sum, item) => sum + item.wearValue, 0) / items.length;

  return ok({
    averageWear: averageWear.toFixed(3),
    predictedTier:
      rarity === "Mil-Spec" ? "Restricted target pool" : "Tier review required",
    collection,
    outputTheme:
      averageWear <= 0.07
        ? "Factory-leaning output odds"
        : "Balanced float with moderate wear spread",
  });
};
...
export const prepareStorageMove = (
  ...
  if (item.storageUnitId === storageUnit.id) {
    return err(
      createInvalidSelectionError(
        "This item is already assigned to the selected storage unit.",
      ),
    );
  }
