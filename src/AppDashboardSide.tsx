...
function SelectedItemSummary(props: SelectedItemSummaryProps): JSX.Element {
  return (
    <div class="mt-5 space-y-5">
      <div
        class={`rounded-3xl bg-gradient-to-br ${props.item.accent} p-5 text-slate-950`}
      >
        <p class="text-xs uppercase tracking-[0.24em] text-slate-900/70">
      ...
      <dl class="grid grid-cols-2 gap-3 text-sm text-slate-300">
        ...
        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <dd class="mt-2 font-medium text-white">
            {props.item.inStorage ? "Storage managed" : "Backpack visible"}
          </dd>
        </div>
      </dl>
      <div class="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
        ...
        <p class="mt-2 leading-6">
          The details panel keeps destructive context, market posture, and
          routing state in one place so users do not have to jump between
          unrelated screens before confirming a plan.
        </p>
      </div>
    </div>
  );
...
function FocusedItemPanel(
  props: Pick<
    AppDashboardProps,
    "dashboard" | "readinessClasses" | "selectedItemId"
  >,
): JSX.Element {
  const selectedItem = () =>
    props.dashboard?.inventory.find((item) => item.id === props.selectedItemId);
    ...
    <article class={`${panel}`}>
      <div class="flex items-center justify-between gap-3">
        ...
        <Show when={selectedItem()}>
          {(item) => (
            <span
              class={`rounded-full px-3 py-1 text-xs font-semibold ${props.readinessClasses[item().readiness]}`}
            >
              {item().readiness}
            </span>
          )}
      ...
      <Show
        ...
        fallback={
          <p class="mt-5 text-sm text-slate-400">
            Pick an inventory card to inspect it here.
          </p>
        }
      >
        {(item) => (
...
function ActivityRail(
  props: Pick<AppDashboardProps, "activityClasses" | "dashboard">,
): JSX.Element {
  return (
    <article id="activity" class={`${panel}`}>
      ...
      <div class="mt-5 space-y-3">
        <For each={props.dashboard?.activity ?? []}>
          {(event) => (
            <article class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div class="flex items-start justify-between gap-3">
                ...
                <span
                  class={`rounded-full px-2.5 py-1 text-xs font-semibold ${props.activityClasses[event.status]}`}
                >
                  {event.status}
                </span>
              </div>
...
function StatusPanel(
  props: Pick<AppDashboardProps, "dashboard" | "toneClasses">,
): JSX.Element {
  return (
    <article class={`${panel}`}>
      <div>
        <p class="text-xs font-semibold uppercase tracking-[0.28em] text-slate-400">
          Connection and compliance status
        </p>
        <h2 class="mt-2 text-2xl font-semibold text-white">
          Operational posture
        </h2>
      ...
      <div class="mt-5 space-y-3">
        <For each={props.dashboard?.statusFlags ?? []}>
          {(status) => (
            <div
              class={`rounded-2xl border p-4 ${props.toneClasses[status.tone]}`}
            >
              <div class="flex items-center justify-between gap-3">
...
export function AppDashboardSide(props: AppDashboardProps): JSX.Element {
  return (
    <aside class="space-y-6">
      ...
      />
      <ActivityRail
        activityClasses={props.activityClasses}
        dashboard={props.dashboard}
      />
      <StatusPanel
        dashboard={props.dashboard}
        toneClasses={props.toneClasses}
      />
    </aside>
  );
}
