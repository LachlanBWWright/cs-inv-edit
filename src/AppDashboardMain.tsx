...
const panel =
  "rounded-3xl border border-white/10 bg-slate-900/70 p-5 shadow-[0_18px_80px_-40px_rgba(56,189,248,0.55)] backdrop-blur";
...
function Metric(props: MetricProps): JSX.Element {
  return (
    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
      <dt class="text-xs uppercase tracking-[0.22em] text-slate-500">
        {props.label}
      </dt>
...
function HeroSummary(
  props: Pick<
    AppDashboardProps,
    "metricCards" | "notice" | "noticeClasses" | "platformLabel"
  >,
): JSX.Element {
  return (
      ...
      <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p class="mt-4 max-w-2xl text-sm text-slate-300 sm:text-base">
            The layout keeps high-risk actions in clear view and scales cleanly
            from wide desktop panes to thumb-friendly mobile stacks.
          </p>
        </div>
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:min-w-[28rem]">
          <For each={props.metricCards}>
...
function PolicyPanel(props: Pick<AppDashboardProps, "dashboard">): JSX.Element {
  return (
    <aside class={`${panel} space-y-4`}>
      ...
      <div class="space-y-3">
        <For each={props.dashboard?.complianceNotes ?? []}>
          {(note) => (
...
function WorkflowOverview(
  props: Pick<AppDashboardProps, "platformLabel" | "workflowCards">,
): JSX.Element {
  return (
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 class="mt-2 text-2xl font-semibold text-white">
              A review-first command center for every operation
            </h2>
          </div>
...
function NextBestActionPanel(
  props: Pick<AppDashboardProps, "nextBestAction">,
): JSX.Element {
  return (
    <article class={`${panel}`}>
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 class="mt-2 text-xl font-semibold text-white">
            What the interface wants you to inspect next
          </h2>
        </div>
      ...
      <div class="mt-5 rounded-3xl border border-white/10 bg-slate-950/70 p-4">
        <p class="text-sm font-medium text-white">
          {props.nextBestAction.title}
        </p>
        <p class="mt-2 text-sm leading-6 text-slate-300">
          {props.nextBestAction.detail}
        </p>
      </div>
      <ul class="mt-4 space-y-2 text-sm text-slate-300">
        <li class="rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
          Use the inventory cards to select an item and surface its readiness
          state before any plan is queued.
        </li>
        <li class="rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
          Keep the trade-up basket at 10 slots for a compact review surface,
          then stage the plan once the collection is coherent.
        </li>
        <li class="rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
          Treat storage pressure as a first-class signal so high-value assets
          stay visible and deliberate.
        </li>
      </ul>
    </article>
  );
...
function InventoryCard(props: {
  item: AppDashboardProps["filteredInventory"][number];
  isActive: boolean;
  inQueue: boolean;
  readinessClasses: Record<string, string>;
  onSelectItem: (id: string) => void;
  onTradeUpToggle: (id: string) => void;
}): JSX.Element {
  return (
    <article
      class={`group rounded-3xl border p-4 text-left transition ${props.isActive ? "border-sky-300/70 bg-sky-400/10 shadow-[0_0_0_1px_rgba(125,211,252,0.2)]" : "border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/8"}`}
    >
      <button
      ...
      >
        <div
          class={`h-28 rounded-2xl bg-gradient-to-br ${props.item.accent} p-4`}
        >
          <div class="flex items-start justify-between gap-3">
            <div>
              <p class="text-xs uppercase tracking-[0.24em] text-slate-950/80">
                {props.item.weapon}
              </p>
            ...
            <span
              class={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${props.readinessClasses[props.item.readiness]}`}
            >
        ...
        <div class="mt-4 flex items-start justify-between gap-4">
          ...
          <div class="text-right text-sm text-slate-300">
            ...
            <p class="mt-1 text-xs text-slate-500">
              Float {props.item.wearValue.toFixed(2)}
            </p>
          </div>
        </div>
        <div class="mt-4 flex flex-wrap gap-2 text-xs text-slate-300">
          ...
          <span class="rounded-full border border-white/10 bg-white/5 px-2.5 py-1">
            {props.item.stickers} stickers
          </span>
          <span class="rounded-full border border-white/10 bg-white/5 px-2.5 py-1">
            {props.item.priceBand}
          </span>
          ...
          <Show when={props.item.inStorage}>
            <span class="rounded-full border border-amber-400/30 bg-amber-500/10 px-2.5 py-1 text-amber-100">
              In storage
            </span>
          </Show>
      ...
      <div class="mt-4 flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => props.onTradeUpToggle(props.item.id)}
          type="button"
        >
    ...
    <article id="inventory" class={`${panel}`}>
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 class="mt-2 text-2xl font-semibold text-white">
            Dense enough for desktop, calm enough for mobile
          </h2>
        </div>
        <FilterButtons
          collectionFilter={props.collectionFilter}
          collections={props.dashboard?.highlightedCollections ?? []}
          onCollectionFilterChange={props.onCollectionFilterChange}
        />
      </div>
      <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <For each={props.filteredInventory}>
          {(item) => (
            <InventoryCard
              item={item}
...
function StickerSummary(props: {
  summary: AppDashboardProps["stickerPlanSummary"];
}): JSX.Element {
  if (!props.summary?.value) {
    return (
      <p class="text-sm text-amber-100">
        {props.summary?.message ?? "Select an item to see sticker guidance."}
      </p>
    );
  }
  ...
  return (
    <div>
      <p class="text-sm font-medium text-white">
        {props.summary.value.confidence}
      </p>
      <ul class="mt-3 space-y-2 text-sm leading-6 text-slate-300">
        <For each={props.summary.value.notes}>
          {(note) => <li>• {note}</li>}
        </For>
      </ul>
    </div>
...
function StickerWorkshopCard(
  props: Pick<
    AppDashboardProps,
    | "onStickerPresetChange"
    | "onStickerReview"
    | "stickerPlanSummary"
    | "stickerPreset"
  >,
  ...
  return (
    <article class={`${panel} xl:col-span-1`}>
      ...
      <div class="mt-5 grid gap-3">
        <For
          each={[
            "Precision alignment",
            "Symmetry lock",
            "Wear-preserving balance",
          ]}
        >
          {(preset) => (
            <button
              class={`rounded-2xl border p-3 text-left ${props.stickerPreset === preset ? "border-sky-300/60 bg-sky-500/10" : "border-white/10 bg-white/5 text-slate-300"}`}
              onClick={() => props.onStickerPresetChange(preset)}
              type="button"
            >
              <p class="font-medium text-white">{preset}</p>
              <p class="mt-1 text-sm text-slate-400">
                Conservative offsets and visible approval states.
              </p>
            </button>
          )}
        </For>
      ...
      <button
        class="mt-5 w-full rounded-2xl bg-sky-400 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300"
        onClick={props.onStickerReview}
        type="button"
      >
...
function TradeUpPlanSummary(props: {
  summary: AppDashboardProps["tradeUpPlanSummary"];
}): JSX.Element {
  if (!props.summary?.value) {
    return (
      <p class="text-sm text-amber-100">
        {props.summary?.message ?? "Select items to plan a trade-up."}
      </p>
    );
  }
      ...
      <div class="grid gap-3 sm:grid-cols-3">
        <div>
          ...
          <p class="mt-2 font-medium text-white">
            {props.summary.value.collection}
          </p>
        </div>
        <div>
          <p class="text-xs uppercase tracking-[0.22em] text-slate-500">
          ...
          <p class="mt-2 font-medium text-white">
            {props.summary.value.averageWear}
          </p>
        </div>
        <div>
          <p class="text-xs uppercase tracking-[0.22em] text-slate-500">
          ...
          <p class="mt-2 font-medium text-white">
            {props.summary.value.predictedTier}
          </p>
        </div>
      </div>
      <p class="mt-4 text-sm text-slate-300">
        {props.summary.value.outputTheme}
      </p>
    </>
  );
}

function TradeUpPlannerCard(
  props: Pick<
    AppDashboardProps,
    "onTradeUpReview" | "tradeUpPlanSummary" | "tradeUpQueue"
  >,
): JSX.Element {
  return (
    <article class={`${panel} xl:col-span-1`}>
      <div class="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 class="mt-2 text-xl font-semibold text-white">
            Ten-slot basket review
          </h2>
        </div>
        <span class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300">
          {props.tradeUpQueue.length}/10 selected
        </span>
      </div>
      <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5 xl:grid-cols-2 2xl:grid-cols-5">
        <For
          each={Array.from(
            { length: 10 },
            (_, index) => props.tradeUpQueue[index],
          )}
        >
          {(itemId, index) => (
            <div class="rounded-2xl border border-white/10 bg-white/5 p-3">
              ...
              <Show
                when={itemId}
                fallback={
                  <p class="mt-3 text-sm text-slate-400">Choose item</p>
                }
              >
                {(value) => (
                  <p class="mt-3 text-sm font-medium text-white">{value()}</p>
                )}
              </Show>
            </div>
          )}
      ...
      <button
        class="mt-5 w-full rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-slate-200"
        onClick={props.onTradeUpReview}
        type="button"
      >
...
function StoragePlanSummary(props: {
  summary: AppDashboardProps["storagePlanSummary"];
}): JSX.Element {
  if (!props.summary?.value) {
    return (
      <p>
        {props.summary?.message ??
          "Select an item and storage unit to plan the move."}
      </p>
    );
  }
  ...
  return (
    <div>
      ...
      <p class="mt-2 text-sm text-slate-400">
        Free slots after move: {props.summary.value.targetFreeSlots}
      </p>
    </div>
  );
}

function StorageUnitButton(props: {
  storageUnit: AppDashboardProps["dashboard"] extends undefined
    ? never
    : NonNullable<AppDashboardProps["dashboard"]>["storageUnits"][number];
  selectedStorageId: string;
  onSelectStorage: (unitId: string) => void;
}): JSX.Element {
  const active = props.storageUnit.id === props.selectedStorageId;
  const fill = Math.round(
    (props.storageUnit.occupied / props.storageUnit.capacity) * 100,
  );

  return (
    <button
      class={`w-full rounded-2xl border p-4 text-left ${active ? "border-sky-300/60 bg-sky-500/10" : "border-white/10 bg-white/5"}`}
      onClick={() => props.onSelectStorage(props.storageUnit.id)}
      type="button"
    >
      ...
      <div class="mt-4 h-2 rounded-full bg-white/10">
        <div
          class="h-2 rounded-full bg-gradient-to-r from-sky-400 to-cyan-300"
          style={{ width: `${fill}%` }}
        />
      </div>
...
function StorageOrchestratorCard(
  props: Pick<
    AppDashboardProps,
    | "dashboard"
    | "onSelectStorage"
  ...
  return (
    <article class={`${panel} xl:col-span-1`}>
      <div class="flex items-start justify-between gap-3">
        ...
        <span class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300">
          Queue first, move later
        </span>
      </div>
      <div class="mt-5 space-y-3">
        <For each={props.dashboard?.storageUnits ?? []}>
          {(storageUnit) => (
            <StorageUnitButton
              storageUnit={storageUnit}
              selectedStorageId={props.selectedStorageId}
              onSelectStorage={props.onSelectStorage}
            />
          )}
        </For>
      </div>
      ...
      <button
        class="mt-5 w-full rounded-2xl bg-emerald-300 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-emerald-200"
        onClick={props.onStorageReview}
        type="button"
      >
      ...
      <section class="grid gap-6 lg:grid-cols-[1.45fr_0.95fr]">
        <div class="space-y-6">
          <HeroSummary
            ...
            notice={props.notice}
            noticeClasses={props.noticeClasses}
            platformLabel={props.platformLabel}
          />
          <div
            class={`rounded-2xl border px-4 py-3 text-sm ${props.noticeClasses}`}
          >
      ...
      <section class="mt-6 grid gap-6 xl:grid-cols-[1.45fr_0.95fr]">
        <div class="space-y-6">
          ...
          <section id="workbench" class="grid gap-6 xl:grid-cols-3">
            <StickerWorkshopCard
              onStickerPresetChange={props.onStickerPresetChange}
              onStickerReview={props.onStickerReview}
              stickerPlanSummary={props.stickerPlanSummary}
              stickerPreset={props.stickerPreset}
            />
            <TradeUpPlannerCard
              onTradeUpReview={props.onTradeUpReview}
              tradeUpPlanSummary={props.tradeUpPlanSummary}
              tradeUpQueue={props.tradeUpQueue}
            />
            <StorageOrchestratorCard
              dashboard={props.dashboard}
              onSelectStorage={props.onSelectStorage}
              onStorageReview={props.onStorageReview}
              selectedStorageId={props.selectedStorageId}
              storagePlanSummary={props.storagePlanSummary}
            />
          </section>
        </div>
      </section>
